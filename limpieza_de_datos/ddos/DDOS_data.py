...
["ddos_id"] = ["DDos" + str(uuid.uuid4().int) ]
    
    now = datetime.now()
    df["Date"] = now.date()
...
    
    cur.executemany("""
        INSERT INTO logs (id, type, indicators, severity, date, time)
        VALUES (%(id)s, %(type)s, %(indicators)s, %(severity)s, %(date)s, %(time)s)
        """, records)
"""
    records_2 = [
        {
...
        id, destination_port, flow_duration, total_fwd_packets, total_backward_packets,
        flow_bytes_s, flow_packets_s, fwd_packet_length_mean, fwd_packet_length_std,
        min_packet_length, max_packet_length, flow_iat_mean, flow_iat_std,
        syn_flag_count, ack_flag_count, down_up_ratio, active_mean, idle_mean,
        label, estandar, description, ataques_cves_tipicos, como_proteger
    )
    VALUES (
        %(id)s, %(destination_port)s, %(flow_duration)s, %(total_fwd_packets)s, %(total_backward_packets)s,
        %(flow_bytes_s)s, %(flow_packets_s)s, %(fwd_packet_length_mean)s, %(fwd_packet_length_std)s,
        %(min_packet_length)s, %(max_packet_length)s, %(flow_iat_mean)s, %(flow_iat_std)s,
        %(syn_flag_count)s, %(ack_flag_count)s, %(down_up_ratio)s, %(active_mean)s, %(idle_mean)s,
        %(label)s, %(estandar)s, %(description)s, %(ataques_cves_tipicos)s, %(como_proteger)s
    )
    """, records_2)
    """
    conn.commit()
